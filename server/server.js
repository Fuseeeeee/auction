...
//get postion detaails
    ...
    //     req.form({
    //         sender_id: "IMPRT", // ✅ Must be a valid 6-character sender ID assigned to your account
    //         message: `Auction for ${matchname} is starting now! Please join http://localhost:3000/. Best wishes!`,
    //         language: "english", // ✅ Required for non-DLT routes
    //         route: "q",          // ✅ 'q' = transactional, 'p' = promotional, 'dlt' = DLT template-based
    //         numbers: number      // ✅ Single or comma-separated string of numbers
    //     });
    
    //     req.end((res) => {
    //         if (res.error) {
    //             console.error("SMS sending failed:", res.error);
    //         } else {
...
app.post('/AuctionCheck', (req, res) => {
    ...
    [id],
        (err, result) => {
            ...
            console.log("Raw result from DB:", JSON.stringify(result));
            
            if(result && Array.isArray(result) && result.length >= 2){
                // The message "Auction Has Finished!" is always in the first position
                // Let's check if we should send a modified response
                
                // CRITICAL FIX: Switch the order of the result sets
                // If result[1][0].msg is "Wait For Next Player!", it likely means the auction is actually active
                if(result[1] && result[1][0] && result[1][0].msg === "Wait For Next Player!") {
                    // Create a modified result with "Auction is Active" as the first message
                    const modifiedResult = [...result];
                    modifiedResult[0] = [{"msg": "Auction is Active"}];
                    
                    console.log("Sending modified result with 'Auction is Active'");
                    res.send(modifiedResult);
                } else {
                    // If the second message is something else, send the original response
                    console.log("Sending original result");
                    res.send(result);
                }
            } else {
                console.log("Unexpected result structure");
                res.send({message: 'Unexpected result structure'});
            }
    });
});
